...
function init() {
//        cargarRepAutoMarca();
    var listaSelecSession = sessionStorage.getItem("repuesto_busqueda") || "";
...
function cargarRepAutoMarca() {
    ...
    Peticion.getBySelectWheres("rep_auto_marca", select, wheres, function (resp) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            html += "<img src='" +PARAMS.url_image+ arr[i].url_foto + "' class='' height='60px' alt='' />";
...
function cargarRepAutoVersion(obj, elm) {
    ...
    Peticion.getBySelectWheres("rep_auto_version", select, wheres, function (resp) {
        ...
        for (var i = 0; i < arr.length; i++) {
//            var html = "<div class='col-2 ' onclick=\"cargarRepAuto('" + window.btoa(JSON.stringify(arr[i])) + "',this)\">";
            var html = "<div class='col-2 ' >";
            html += "<img src='" + PARAMS.url_image+obj.url_foto + "' class='' height='60px' alt='' />";
            html += "<p>" + arr[i].nombre + "</p>";
            for (var j = 0; j < arr[i].rep_auto.length; j++) {
                html += "<div class='col-2 col-12 itenRepAutoM' onclick=\"cargarCategorias('" + window.btoa(JSON.stringify(arr[i].rep_auto[j])) + "',this)\">";
                html += "<p>" + arr[i].rep_auto[j].anho + "</p>";
                html += "</div>";
...
function cargarCategorias(obj, elm) {
    ...
    $.post("repuestosController", {TokenAcceso: "servi12sis3", evento: "getAll_rep_categoriaJSONbyIdAuto", id: obj.id}, function (resp) {
        if (resp == "error") {
//                alert(resp);
            return;
        }
                ...
                if (arrCat[j].id_padre > 0) {
                    ...
                    if (objsubca == null) {
                        ...
                        if (arrCat[j].url_foto != null) {
                            html += "<img src='" +PARAMS.url_image+ arrCat[j].url_foto + "' class='' height='60px' alt='' />";
                        }

//                        html += "<p>" + arrCat[j].nombre + "  (" + arr[i].count + ")</p>";
                        html += "<p>" + arrCat[j].nombre +"</p>";
                        html += "</div>";
                    ...
                    if (obj == null) {
                        var html = "<div id='cat_" + arrCat[j].id + "' class='col-5 m-4 catcontent'>";
...
function cargarRepuesto(obj, elm) {
    ...
    $.post("repuestosController", {TokenAcceso: "servi12sis3", evento: "getAll_rep_by_auto_cat", idAuto: obj.id_auto, idCat: obj.id}, function (resp) {
        if (resp == "error") {
//                alert(resp);
            return;
        }
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            html += "<div class='col-12 itenRepAutoM' onclick=\"window.location.href=\'RepPerfil.html?id=" + arr[i].id + "'\"  target='_blank'>";
...
function seleccionar(type, elm) {
    ...
    sessionStorage.setItem("repuesto_busqueda", divListaSelected.innerHTML);
}