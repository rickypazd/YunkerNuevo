...
var urlAlmacen = "almacenController";
var cantidad = 10;
var pagina = 0;
...
function Lista_repuesto() {
    ...
    pagina = $("#list").data("pagina");

    var limite = $("#resultadosAlmacen").data("cantidad");
    if ((pagina * 10) >= limite) {
        return;
    }
    if (enPedido) {
    ...
    $.post(urlAlmacen, {evento: "getAll_almacen_pagination", busqueda: busqueda, cantidad: 10, pagina: pagina, TokenAcceso: "servi12sis3"}, function (resp) {
        // cerrar_progress();
        enPedido = false;
...
function editar(itn) {
    ...
    window.location.href = "AlmacenCanv.html?id=" + data.id;
...
function ok_eliminar(itn) {
    var id = $(itn).data("id");
    mostrar_progress();
    $.post(urlAlmacen, {evento: "eliminar_almacen", id: id, TokenAcceso: "servi12sis3"}, function (resp) {
        cerrar_progress();
        if (resp != null) {
            var obj = $.parseJSON(resp);
            if (obj.estado != "1") {
