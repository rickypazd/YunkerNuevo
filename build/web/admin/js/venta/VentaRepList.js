    ...
    $.post(url, {evento: "getAll_repuesto_paginationJSON", busqueda: busqueda, cantidad: 10, pagina: pagina, TokenAcceso: "servi12sis3"}, function (resp) {
        ...
        if (resp != null) {
            ...
            if (obj.estado != "1") {
                ...
                var arr = $.parseJSON(json.data);
                
                $("#resultados").html("(" + json.cantidad + ")");

                if (json.count == 0) {
                    return;
                }
                $("#resultados").data("cantidad", json.cantidad);
                $.each(arr, function (i, obj) {
                    var cant = obj.obj.cantidad || 0;
                    obj.obj.repuesto.cantidad = cant;
                    var html = "<div data-obj='" + JSON.stringify(obj.obj) + "' class='list-group-item list-group-item-action flex-column align-items-start'>";
                    ...
                    html += "                         <p style='font-size:10px'><b>Almacen: </b>"+(obj.obj.almacen || " -- ")+"</p>";
                    ...
                    html += "                         <h6>" + (obj.obj.repuesto.cantidad || 0) + "</h6>";
                    html += "                     </div>";
                    html += "                     <div class='col-2'>";
...
html += "                        <div class='col-4 btnAction btnVer d-flex justify-content-center  align-items-center' onclick='ver(this);'> <div> <img src='img/cheuron.svg' alt=''/><p>Ver</p></div></div>";
                    html += "                        <div class='col-4 btnAction btnEliminar d-flex justify-content-center  align-items-center' onclick='AggArticulo(this);'>  <div><img src='img/cheuron.svg' alt=''/><p>Add</p></div></div>";
                    html += "                    </div>";
                    html += "                     </div>";
                    html += "                 </div>";
                    html += "            </div>";
